// File: src/js/theme-switch.js

// Theme switching functionality
document.addEventListener('DOMContentLoaded', function() {
    // The toggle switch will be created by theme-switch-component.js
    // We just need to wait for it to be available
    const checkForToggleSwitch = setInterval(function() {
        const toggleSwitch = document.querySelector('#checkbox');
        
        if (toggleSwitch) {
            clearInterval(checkForToggleSwitch);
            
            // Always use a dark theme by default, only change if explicitly set to light in localStorage
